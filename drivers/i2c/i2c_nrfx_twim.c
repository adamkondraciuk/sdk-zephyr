...
static void twim_irq_handler(const struct device *dev) {
	const struct i2c_nrfx_twim_common_config *config = dev->config;

	nrfx_twim_irq_handler(&config->twim);
}
...
#define CONCAT_BUF_SIZE(idx)						           \
	COND_CODE_1(DT_NODE_HAS_PROP(DT_DRV_INST(idx), zephyr_concat_buf_size),    \
		    (DT_PROP(DT_DRV_INST(idx), zephyr_concat_buf_size)), (0))
#define FLASH_BUF_MAX_SIZE(idx)					        	   \
	COND_CODE_1(DT_NODE_HAS_PROP(DT_DRV_INST(idx), zephyr_flash_buf_max_size), \
		    (DT_PROP(DT_DRV_INST(idx), zephyr_flash_buf_max_size)), (0))
...
#define I2C_NRFX_TWIM_DEVICE(idx)					       \
	...
	BUILD_ASSERT(XXX_I2C_FREQUENCY(idx) !=				       \
		     I2C_NRFX_TWIM_INVALID_FREQUENCY,			       \
		     "Wrong I2C " #idx " frequency setting in dts");	       \
	static void irq_connect##idx(void)				       \
	{								       \
		IRQ_CONNECT(DT_INST_IRQN(idx), DT_INST_IRQ(idx, priority),     \
			    twim_irq_handler, DEVICE_DT_INST_GET(idx), 0);     \
	}								       \
	...
	static struct i2c_nrfx_twim_common_config twim_##idx##z_config = {     \
		...
		.twim_config = {					       \
			.skip_gpio_cfg = true,				       \
			.skip_psel_cfg = true,				       \
			.frequency = XXX_I2C_FREQUENCY(idx),		       \
		},							       \
...
#define I2C_MEMORY_SECTION(idx)						       \
	COND_CODE_1(XXX_I2C_HAS_PROP(idx, memory_regions),		       \
