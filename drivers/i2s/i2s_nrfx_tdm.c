				...
				const struct i2s_config *tdm_cfg)
{
	...
	const uint32_t src_freq =
		...
		: 32*1000*1000UL;
	config->mck_setup = NRF_TDM_MCK_DIV_2;
...
static void tdm_start(struct tdm_nrfx_drv_data *drv_data, tdm_buffers_t const * p_initial_buffers)
{
...
static void next_buffers_set(struct tdm_nrfx_drv_data *drv_data,
				...
				tdm_buffers_t *next)
{
	...
	next_buffers_set(drv_data, next);
	return true;
}
			      ...
			      const struct i2s_config *tdm_cfg)
{
	...
	if (drv_data->state != I2S_STATE_READY) {
		LOG_ERR("Cannot configure in state: %d\n", drv_data->state);
		return -EINVAL;
	}
	...
	case I2S_FMT_DATA_FORMAT_RIGHT_JUSTIFIED:
		...
		break;
	default:
		LOG_ERR("Unsupported data format: 0x%02x\n", tdm_cfg->format);
		return -EINVAL;
	}
	...
	if ((tdm_cfg->format & I2S_FMT_DATA_ORDER_LSB) ||
		...
		LOG_ERR("Unsupported stream format: 0x%02x\n", tdm_cfg->format);
		return -EINVAL;
	}
...
#warning extend me
	...
	if ((tdm_cfg->options & I2S_OPT_BIT_CLK_SLAVE) &&
		...
		LOG_ERR("Unsupported operation mode: 0x%02x\n", tdm_cfg->options);
		return -EINVAL;
	}
	nrfx_cfg.sck_setup = NRF_TDM_SCK_DIV_125;
	...
	if (nrfx_cfg.mode == NRF_TDM_MODE_MASTER ||
	    (nrf_tdm_mck_pin_get(drv_cfg->p_reg) & TDM_PSEL_MCK_CONNECT_Msk)
		...
		nrf_tdm_mck_set(drv_cfg->p_reg, true);
		nrfx_cfg.mck_setup = NRF_TDM_MCK_DIV_2;
	} else {
		nrf_tdm_mck_set(drv_cfg->p_reg, false);
		drv_data->request_clock = false;
	}

	if ((tdm_cfg->options & I2S_OPT_LOOPBACK) ||
	    (tdm_cfg->options & I2S_OPT_PINGPONG)) {
		LOG_ERR("Unsupported options: 0x%02x\n", tdm_cfg->options);
		return -EINVAL;
	}
			  ...
			  void *mem_block, size_t size)
{
	...
	if (drv_data->state != I2S_STATE_RUNNING &&
		...
		LOG_ERR("Cannot write in state: %d\n", drv_data->state);
		return -EIO;
	}
...
static void tdm_uninit(struct tdm_nrfx_drv_data *drv_data)
{
	NRF_TDM_Type * p_reg = drv_data->p_reg;
	tdm_data_t * p_cb = drv_data->p_cb;
	tdm_stop(p_reg);
...
#define TDM_IRQ_CONFIGURE(idx, isr_handler) \
	do {								       \
		IRQ_CONNECT(DT_IRQN(TDM(idx)), DT_IRQ(TDM(idx), priority), \
			    isr_handler, DEVICE_DT_GET(TDM(idx)), 0);	       \
		irq_enable(DT_IRQN(TDM(idx)));			       \
	} while (false)

#define TDM_NRFX_DEVICE(idx)						     \
	static tdm_data_t tdm##idx##data; \
	...
	static struct tdm_nrfx_drv_data tdm_nrfx_data##idx = {		     \
		...
		.p_reg = tdm_nrfx_cfg##idx.p_reg,				     \
		.p_cb = &tdm##idx##data, \
	};								     \
	static int tdm_nrfx_init##idx(const struct device *dev)		     \
	{ \
		TDM_IRQ_CONFIGURE(idx, tdm_##idx##_irq_handler);						     \
		const struct tdm_nrfx_drv_cfg *drv_cfg = dev->config;	     \
